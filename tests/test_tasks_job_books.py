...
class TestJobBookTasks:
    ...
    async def test_insert_str_success(self):
        ...
        with patch("app.tasks.job_books.bootstrap_runtime", new=AsyncMock(return_value=(db, redis, pdata))), \
            patch("app.tasks.job_books.cleanup_runtime", new=AsyncMock()) as cleanup_mock, \
    ...
    async def test_delete_failure_marks_failed(self):
        ...
        with patch("app.tasks.job_books.bootstrap_runtime", new=AsyncMock(return_value=(db, redis, MagicMock()))), \
            patch("app.tasks.job_books.cleanup_runtime", new=AsyncMock()) as cleanup_mock, \
    ...
    async def test_insert_file_missing_batch_item_id_fails_fast(self):
        with patch("app.tasks.job_books.bootstrap_runtime", new=AsyncMock()) as bootstrap_mock:
