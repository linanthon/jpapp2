...
from handlers.helpers import str_2_byte, get_dbhandling, view_count_cache
...
def handle_view_books(star: bool = False, limit: int = DEFAULT_LIMIT, page: int = 1) -> Tuple[List[dict], int]:
    """
    Handle viewing a list of `limit` JP words with their 1st EN meaning.
...
def handle_view_specific_book(book_id: int) -> dict:
    """
    Handle viewing a JP word with `sentence_limit` amount of sentence examples.
    """
    db = get_dbhandling()
